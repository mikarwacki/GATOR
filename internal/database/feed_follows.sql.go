...
const deleteFeedByUserIdAndUrl = `-- name: DeleteFeedByUserIdAndUrl :exec
DELETE FROM feed_follows 
USING feeds 
WHERE feeds.url = $1 and feed_follows.user_id = $2
`

type DeleteFeedByUserIdAndUrlParams struct {
	Url    string
	UserID uuid.UUID
}

func (q *Queries) DeleteFeedByUserIdAndUrl(ctx context.Context, arg DeleteFeedByUserIdAndUrlParams) error {
	_, err := q.db.ExecContext(ctx, deleteFeedByUserIdAndUrl, arg.Url, arg.UserID)
	return err
}
